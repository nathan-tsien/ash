...
| Companion Shadow LogoMark | `LogoMark` icon in `@ash/ui`, sidebar home mark, assistant identity row, marketing/auth brand lockups, future favicon/app icon source | Cultivate; vector-only, no hand/face/generic AI icon |
...
| v2.1.1 | 2026-07-05 | PATCH — LogoMark placement broadened across assistant message identity, marketing/auth brand lockups, and a larger workbench sidebar home mark so brand memory is consistent across product surfaces. |
