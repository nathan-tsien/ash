...
| P2.6 | praxis 0.4.0 history chronological ordering — `MessagePage.items` are ascending (oldest-first); history projection drops the per-page reverse and the re-attach loop prepends older pages so multi-page catch-up stays chronological (ADR-0019). Doc-string-only contract diff; server-side pagination correctness fix inherited. | — | **Committed** |
